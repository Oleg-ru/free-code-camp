...
function displayBooks(books) {
  return books
    .map((book) => {
      return `Book: ${book.title} author: ${book.author} and pages ${book.pages}`;
    })
    .join('\n');
}
...
/**
 *
 * @param books {array} Массив объектов книг
 * @returns {array} Массив значений about
 */
...
function getTotalPages(books) {
  return books.reduce(
    (accumulator, currentValue) => accumulator + currentValue.pages,
    0
  );
}
